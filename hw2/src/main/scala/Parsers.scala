      ...
      case _ => 
        ctx.getChildCount match {
          ...
          case 3 => 
            ...
            case "==" => Equal(visit(ctx.getChild(0)), visit(ctx.getChild(2)))
            case "<" => Less(visit(ctx.getChild(0)), visit(ctx.getChild(2)))
...
class IntVisitor extends IntParserAntlr4BaseVisitor[IntExpr] {
  ...
  override def visitExpr(ctx: IntParserAntlr4.ExprContext): IntExpr = {
    ...
    ctx.getChild(0).toString match {
      ...
      case "pi" => IntPi(visit(ctx.expr(0)), visit(ctx.expr(1)), visit(ctx.expr(2)))
